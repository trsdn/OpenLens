...
///
/// The mach service names must be prefixed with the app group, otherwise the
/// sandbox refuses to let the extension vend them.
public enum OpenLensID {
    ...
    /// Our own app -> extension frame transport.
    public static let frameMachServiceName = "\(appGroup).frames"
